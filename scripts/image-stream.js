...
var config = {
  ...
  'jquery_lazyload': '//cdnjs.cloudflare.com/ajax/libs/jquery.lazyload/1.9.1/jquery.lazyload.min.js',
...
hexo.extend.tag.register('figure', function(args){
  ...
  result += '<figcaption >' + hexo.render.renderSync({text: title, engine: 'markdown'}).replace(/<p>/, '').replace(/<.p>/, '') + '</figcaption>' + '</a>';
  result += '</figure>';
  return result;
});
...
hexo.extend.tag.register('part', function(args){
  ...
  var result = '<part>';
  result += '<div classs="img-click" data="' + imgUrl + '">'
  result += '<img class="hexo-image-steam-lazy nofancy" alt="' + title + '" src="' + placeholder + '" data-original="' + imgUrl + '"/></div>';
  result += '<noscript><img src="' + imgUrl + '"/></noscript>';
  result += '<a href="' + wordUrl + '" target="_blank" rel="external" style="text-decoration: none">'
  result += '<figcaption>'+ comment +'</figcaption>';
